...
class VulnragRetriever:
    ...
    def get_top_k_structure(self, code: str, pattern_type: str, top_k: int = 10) -> List[Dict]:
        """
        Get top-k results from structural similarity search
        
        Args:
            code: Source code to analyze
            pattern_type: "VULN" or "PATCH"
            top_k: Number of top results
            
        Returns:
            List of dictionaries with id and rank
        """
        struct_vec, _, _ = self.extract_query_features(code)
        
        if pattern_type == "VULN":
            struct_data = self.vuln_struct
            scaler = self.vuln_struct_scaler
        else:
            struct_data = self.patch_struct
            scaler = self.patch_struct_scaler
            
        structural_scores = cosine_similarity(
            scaler.transform(struct_vec), 
            struct_data
        ).flatten()
        
        top_indices = np.argsort(structural_scores)[-top_k:][::-1]
        
        results = []
        for rank, idx in enumerate(top_indices):
            entry_id = list(self.kb2_data.keys())[idx]
            results.append({
                "id": f"{entry_id}_{pattern_type.lower()}",
                "entry_id": entry_id,
                "pattern_type": pattern_type,
                "rank": rank,
                "score": float(structural_scores[idx])
            })
        
        return results

    def get_top_k_tfidf(self, code: str, pattern_type: str, top_k: int = 10) -> List[Dict]:
        """
        Get top-k results from TF-IDF similarity search
        ...
        Returns:
            List of dictionaries with id and rank
        """
        _, semantic_text, _ = self.extract_query_features(code)
        
        if pattern_type == "VULN":
            tfidf_vectorizer = self.vuln_tfidf_vectorizer
            tfidf_matrix = self.vuln_tfidf_matrix
        else:
            tfidf_vectorizer = self.patch_tfidf_vectorizer
            tfidf_matrix = self.patch_tfidf_matrix
            
        tfidf_scores = cosine_similarity(
            tfidf_vectorizer.transform([semantic_text]),
            tfidf_matrix
        ).flatten()
    ...
    def get_top_k_embedding(self, code: str, pattern_type: str, top_k: int = 10) -> List[Dict]:
        """
        Get top-k results from embedding similarity search
        ...
        Returns:
            List of dictionaries with id and rank
        """
        _, _, embedding = self.extract_query_features(code)
        
        if pattern_type == "VULN":
            embeddings_data = self.vuln_embeddings
        else:
            embeddings_data = self.patch_embeddings
        ...
        top_indices = np.argsort(embedding_scores)[-top_k:][::-1]
    ...
    def fetch_items_by_ids(self, ids_with_metadata: List[Dict]) -> List[RetrievalResult]:
        """
        Fetch full items by their IDs with metadata
        
        Args:
            ids_with_metadata: List of dictionaries with id, entry_id, pattern_type, etc.
        ...
        for item_meta in ids_with_metadata:
            entry_id = item_meta["entry_id"]
            pattern_type = item_meta["pattern_type"]
            
            if entry_id not in self.kb2_data:
                continue
                
            entry_data = self.kb2_data[entry_id]
            
            # Get appropriate features based on pattern type
            if pattern_type == "VULN":
                features_key = "vuln_features"
            else:
                features_key = "patch_features"
            
            result = RetrievalResult(
                entry_id=entry_id,
                hybrid_score=item_meta.get("rrf_score", 0.0),
                structural_score=0.0,  # Will be filled later if needed
                tfidf_score=0.0,       # Will be filled later if needed
                embedding_score=0.0,   # Will be filled later if needed
                cve_id=entry_data.get('cve', 'Unknown'),
                cwe_id=entry_data.get('cwe', 'Unknown'),
                pattern_type=pattern_type,
                structural_features=entry_data.get(features_key, {}).get('structural_features', {}),
                semantic_features=entry_data.get(features_key, {}).get('semantic_features', {}),
    ...
    def retrieve_with_rrf(self, code: str, pattern_type: str, top_k: int = 10) -> List[RetrievalResult]:
        """
        Perform RRF-based retrieval combining structural, TF-IDF, and embedding searches
        ...
        Returns:
            List of RetrievalResult objects ranked by RRF scores
        """
        self.logger.debug(f"Starting RRF retrieval for {pattern_type} patterns")
        
        # 1. Get top-k from each base (with rank)
        results_by_index = {}
        results_by_index["structure"] = self.get_top_k_structure(code, pattern_type, top_k)
        results_by_index["tfidf"] = self.get_top_k_tfidf(code, pattern_type, top_k)
        results_by_index["embedding"] = self.get_top_k_embedding(code, pattern_type, top_k)
        
        # 2. Build a unified result set with RRF scores
        rrf_scores = defaultdict(float)
        item_metadata = {}  # Store metadata for each item
        
        for base_name, results in results_by_index.items():
            for result_item in results:
                item_id = result_item["id"]
                rank = result_item["rank"]
                
                # RRF formula: 1 / (rank + 1)
                rrf_scores[item_id] += 1.0 / (rank + 1)
                
                # Store metadata (only once per item)
                if item_id not in item_metadata:
                    item_metadata[item_id] = {
                        "entry_id": result_item["entry_id"],
                        "pattern_type": result_item["pattern_type"]
                    }
        
        # 3. Reconstruct final ranked list
        ranked_items = sorted(rrf_scores.items(), key=lambda x: -x[1])
        top_items = ranked_items[:top_k]
        
        # 4. Prepare metadata with RRF scores
        items_with_rrf = []
        for item_id, rrf_score in top_items:
            meta = item_metadata[item_id].copy()
            meta["id"] = item_id
            meta["rrf_score"] = rrf_score
            items_with_rrf.append(meta)
        
        # 5. Retrieve full items by ID
        final_results = self.fetch_items_by_ids(items_with_rrf)
        
        self.logger.debug(f"RRF retrieval completed: {len(final_results)} results for {pattern_type}")
    ...
    def search_vuln_patterns(self, code: str, top_k: int = 10) -> List[RetrievalResult]:
        """
        Search for VULN patterns using RRF-based hybrid approach
        ...
        """
        self.logger.debug(f"Searching VULN patterns with RRF (top_k={top_k})")
        return self.retrieve_with_rrf(code, "VULN", top_k)

    def search_patch_patterns(self, code: str, top_k: int = 10) -> List[RetrievalResult]:
        """
        Search for PATCH patterns using RRF-based hybrid approach
        ...
        """
        self.logger.debug(f"Searching PATCH patterns with RRF (top_k={top_k})")
        return self.retrieve_with_rrf(code, "PATCH", top_k)
    ...
    def hybrid_retrieval(self, code: str, top_k: int = 10, 
        ...
        retrieval_results = VulnragRetrievalResults(
            ...
            retrieval_metadata={
                ...
                "rrf_formula": "sum(1/(rank+1)) across structural+tfidf+embedding",
                "retrieval_method": "double_indexation_rrf_based"
            },
